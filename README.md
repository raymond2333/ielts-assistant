...
- **背单词**：雅思核心词库 + AI 查词 + 收藏。
- **管理员后台**：管理员可查看现有用户、筛选和删除练习记录、删除用户。
...
- **管理员后台**（`/admin`）：管理员账号可管理用户和练习记录

> `FLASK_SECRET_KEY` 同时用于跨版本免登录的 HMAC token 签名，两个版本需设置相同的值。

### 管理员入口

Flask Beta 版支持管理员后台。默认管理员用户 ID 为 `admin`，也可以通过环境变量配置多个管理员：

```bash
export ADMIN_USER_IDS=admin,teacher
```

用管理员 ID 注册并登录后，学习总览会出现「管理员后台」入口。管理员可以查看用户列表、筛选所有用户的练习记录、删除练习记录和删除用户。普通用户只能删除自己的练习记录。
