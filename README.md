...
- **背单词**：雅思核心词库 + AI 查词 + 收藏。
- **管理员后台**：独立的纯管理入口，可查看/修改用户资料、密码、API 配置、管理员权限和练习记录。
...
- 稳定版仅保留文本练习体验；语音朗读、语音输入、录音评分统一由 Flask Beta 版提供。
...
录音转写只在服务器本地完成，不消耗各模型 API。若服务器未安装本地语音识别组件或录音未识别到文字，可以直接在文本框输入回答后提交评分。
...
- **口语练习**（`/speaking`）：Part 1/2/3，含计时器、AI 反馈、关键词生成答案、语音朗读、语音输入和录音评分
...
- **管理员后台**（`/admin`）：管理员账号可管理用户资料、密码、API、权限和练习记录

> `FLASK_SECRET_KEY` 同时用于跨版本免登录的 HMAC token 签名，两个版本需设置相同的值。

### 管理员入口

Flask Beta 版支持管理员后台，访问地址为：
...
```text
http://127.0.0.1:8600/admin
```

默认管理员用户 ID 为 `admin`，但系统**没有预置默认管理员密码**。管理员账号需要像普通用户一样先在注册页注册：

1. 打开 Flask Beta 版登录页。
2. 切换到「注册」。
3. 用户 ID 填写 `admin`。
4. 自己设置登录密码。
5. 注册后会自动登录，并拥有管理员权限。

也可以通过环境变量配置多个管理员用户 ID：

```bash
export ADMIN_USER_IDS=admin,teacher
```

用管理员 ID 注册并登录后，学习总览会出现「管理员后台」入口，也可以直接访问 `/admin`。管理员后台是纯管理入口，不提供口语、写作、背单词等学习功能导航。

管理员可以：

- 查看现有用户列表，点击用户名进入该用户详情。
- 查看和修改成员个人信息、四项雅思水平、目标分数、学习目标和考试日期。
- 修改成员登录密码。
- 查看和修改成员各模型 API Key、当前模型、模型名称和 Base URL。
- 将其他成员设为管理员，或取消数据库中配置的管理员权限。
- 筛选、查看和删除用户练习记录。
- 删除用户。

普通用户只能删除自己的练习记录。通过环境变量 `ADMIN_USER_IDS` 配置的管理员始终拥有管理员权限，不能仅通过后台取消；若要取消，需要修改服务器环境变量并重启应用。
...
> Streamlit 稳定版不再提供语音朗读、语音输入和录音评分；这些能力保留在 Flask Beta 版中，避免旧版浏览器组件不稳定影响练习体验。
...
点击"📅 生成详细学习计划"后，系统会结合用户目标、弱项和训练历史记录，由 AI 生成个性化学习计划（按周排列详细安排）。学习计划可持久化保存，但不会显示在普通练习记录列表中。
...
语音朗读、语音输入、本地录音转文字和录音评分现在是 Flask Beta 版独有功能。Streamlit 稳定版只保留文本输入、AI 批改、题目生成和学习记录回看。录音评分会先在服务器本地转文字，再把转写文本提交给当前模型评分；若本地未识别到文字，可以直接输入文本后评分。

### 学习计划可以重新生成吗？

可以。已生成的学习计划可持久化保存，但会从普通练习记录列表中过滤掉；点击「🔄 重新生成」可基于最新训练记录重新由 AI 生成。
